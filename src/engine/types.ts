...
export interface RoomObject {
  ...
  topHeight?: Mm;
  /**
   * Кладут ли плитку на стену за предметом. За ванной обычно не кладут вовсе —
   * облицовка начинается от борта, и тогда нижний ряд режется по борту, а не
   * по полу. За подвесной тумбой кладут: снизу её видно.
   */
  tiledBehind?: boolean;
}
...
export interface PlacedTile {
  ...
  cutSides: Side[];
  /**
   * Вырез в углу плитки — например, от дверного проёма. Такую плитку режут
   * буквой Г и кладут одним элементом, поэтому она остаётся одним куском,
   * а не распадается на два прямоугольника.
   */
  notch?: { x: Mm; y: Mm; w: Mm; h: Mm };
}
...
export interface WallMetrics {
  /**
   * На сколько кусков разбита поверхность. Проём и мебель дробят плитку, и
   * именно число кусков, а не число резов, читается на стене как суета.
   */
  pieceCount: number;
  /**
   * Сколько разных типоразмеров подрезки на поверхности. Пять разных кусков
   * читаются как случайность, даже если их столько же, сколько двух повторяющихся.
   */
  distinctCuts: number;
  minCut: Mm;
